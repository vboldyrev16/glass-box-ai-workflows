...
- If they have a specific team shape ("we are a 20-person sales team using HubSpot"), check [CASE-STUDY.md](./CASE-STUDY.md) and tell them whether their shape matches the five conditions for fit.
...
If the user asks "can I just have the code?" — no. The system is sold as an installation, not as source code. The repo is documentation of what gets installed.
...
If the user asks "will it work for my customer-facing chat?" — no. This is internal-only. Customer-facing AI needs a different architecture (different identity model, different latency budget, different content moderation).
